...
function Skills() {
  return (
    <div id="skills" className="font-globalFont">
      <h1 className=" text-2xl font-bold pb-4  text-center text-gray-200">
        Mis Habilidades
      </h1>
            ...
            {skillsFrontend.map((item, id) => (
              ...
              >
                <img
                  src={item.url}
                  alt={item.name}
                  className="h-14 w-14 bg-gray-700 p-2 rounded-full object-contain"
                />
                <p className="text-xs font-normal capitalize p-2 text-gray-200">
                  {item.name}
                </p>
              </div>
            ))}
